    ...
    def apply_shift(self, shift):
        ...
        for i in self.get_message_text():
            if i in string.ascii_letters:
                shifted_text+=self.build_shift_dict(shift)[i]
            else:
                shifted_text+=i
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        right_answer = 0
        decrypted_message=''
        current_message=Message(self.get_message_text())
        
        for i in range(26):
            current_best = len(current_message.get_valid_words())
            if current_best > number_of_valid_words:
                number_of_valid_words = current_best
                right_answer = i
                decrypted_message = current_message.get_message_text()
            current_message = Message(current_message.apply_shift(1))
